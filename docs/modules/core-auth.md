...
- **Backend** (`backend/app/core/auth/`):
  ...
  - `services.py`: `authenticate()`, which always checks an argon2 hash, even for an unknown email, so timing doesn't reveal which emails exist. It rejects inactive or deleted users and rehashes outdated hashes. Also `get_active_user()` and `normalize_email()`.
