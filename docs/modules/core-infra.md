...
- **Planned core packages** (not created yet; each is created, with its dependencies, by the feature that first needs it):
  - `core/ai/gemini_client.py`: the ONLY module allowed to call Gemini (google-genai). Regex-scrubs PAN/GSTIN/email/phone/Aadhaar-like patterns from every prompt and logs a warning when it finds one; model names from new `GEMINI_*` variables (rule 1).
  - `core/email/`: `send_email(to, subject, body)` over SMTP (new `MAIL_*` variables); in development every email lands in Mailpit (http://localhost:8025).
  - `core/notifications/`: `Notification` model, `notify(user_id, ...)` (also sends the email) and the tray API.
  - `core/ocr/`: Tesseract (pytesseract) for images and PyMuPDF for PDFs (PyMuPDF only; use `import pymupdf`). Local only (rule 2); tests that need Tesseract use `@pytest.mark.requires_tesseract`.
  - `core/storage/`: save/load uploaded files, encrypted at rest with Fernet (cryptography); only metadata goes in the database.
  - `core/security/` has password hashing today; `EncryptedString` and `blind_index()` come with the first encrypted field (rule 4).
