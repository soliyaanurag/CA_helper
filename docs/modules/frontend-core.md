...
- **Works:** Vite + React app in plain JavaScript (`.js`/`.jsx`) with Tailwind and shadcn/ui (`badge`, `button`,
...
- `apiFetch()`, `ApiRequestError`, `errorMessage()` from `@/api/client`: make every call with
...
- `label()` and the `*_LABELS` maps from `@/lib/labels`: display text for enum codes (must match
  `docs/DATA_MODEL.md` "Status values"); add a map when a new enum reaches the UI
- `cn()` from `@/lib/utils` and shadcn/ui components from `@/components/ui/*` (add more with
...
- Every API call goes through `apiFetch()` (except `useHealth()`, whose 503 body is data, not an error)
