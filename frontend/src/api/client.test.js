...
import { ApiRequestError, errorMessage, unwrap } from "./client";

function call(status, body) {
...
describe("unwrap", () => {
  ...
  it("throws ApiRequestError with the standard error body's fields", async () => {
    ...
    const error = await unwrap(call(403, body)).catch((e) => e);
  ...
  it("still throws ApiRequestError when the body is not the standard shape", async () => {
    const error = await unwrap(call(502, "Bad gateway")).catch((e) => e);
