...
**Product (report title "ComplianceConnect"):** a two-sided web platform for Indian MSMEs, gig workers and small businesses. It tells each business which filings apply and when (ITR, GSTR-1, GSTR-3B, CMP-08, GSTR-4, 24Q, 26Q),
...
- Backend: Python 3.12 (conda env `ca-helper`), Flask 3 app factory, flask-smorest (OpenAPI, Swagger at `/api/docs`),
  Flask-SQLAlchemy 2.x style + Flask-Migrate, JWT, CORS, Limiter, Postgres 16 (psycopg 3), argon2, APScheduler in a
  **separate worker process**. Added with the feature that needs them: Fernet + HMAC blind index, pgvector,
  google-genai, Tesseract + PyMuPDF (PDFs: PyMuPDF only).
- Frontend: React + Vite + TypeScript, React Router, TanStack Query, Tailwind + shadcn/ui, React Hook Form + Zod,
  openapi-typescript + openapi-fetch (types generated, never hand-written), Vitest + RTL. FullCalendar and Recharts
  are added with the first page that needs them.
- **Dev mode (daily):** `make infra` (Postgres + Mailpit in Docker) + `make dev-backend` / `dev-worker` / `dev-frontend`.
  **Manual (for people, not Claude):** `conda activate ca-helper` + `cd backend && python main.py` + `npm run dev`.
  Claude always uses Make targets or `conda run`. All read the root `.env`. Ports: API 8000, Vite 5173, Postgres
  5432, Mailpit UI 8025. Ask before substituting any tool; record decisions in `docs/DECISIONS.md`.
...
2. Conda installs only `python=3.12`, `tesseract`, `nodejs=22`, `pip`. **All Python packages come from pip via
   `backend/requirements.txt` / `requirements-dev.txt`** (CI uses the same files). Never `conda install` a Python
   library; never pip-install anything that isn't in a requirements file. npm packages come from `frontend/package-lock.json`.
...
5. OCR tests use `@pytest.mark.requires_tesseract`, which skips with a clear message if Tesseract is missing.

## Non-negotiable rules
0. **Simplicity first:** build the simplest thing that works and is explainable in 1–2 sentences. Don't add
...
10. **Layering:** routes parse, call one service function, serialize; services hold the logic and each public one
   ...
   Enums are `StrEnum` with lowercase snake_case values stored via `str_enum()`. See `docs/PATTERNS.md`.

## Key commands
`make setup` · `make env-update` · `make infra` / `infra-down` · `make dev-backend` / `dev-worker` / `dev-frontend` ·
`make test` · `make lint` / `format` · `make migrate` · `make migration name="<module>: <msg>"` · `make seed` · `make gen-api`

## Shared context
Everyone runs their own Claude session. This file loads automatically; **module docs** (`docs/modules/<m>.md`:
Purpose · What exists now · Tables · Endpoints · Service functions other modules call · Depends on · Contracts ·
Known issues) are read explicitly. **The repo is the single source of truth:** anything decided in chat that affects
others goes into `docs/` (module doc, `DECISIONS.md`, `DATA_MODEL.md` or `API_CONVENTIONS.md`) in the same PR.
...
5. Summarize what changed on main that matters here: `git log --oneline -20 origin/main -- docs backend/migrations
   backend/app/core frontend/src/core` plus modules under "Depends on".
6. Env check: env `ca-helper` exists (else tell the user to run `make setup`); `environment.yml`/`requirements*`
   ...
   `make infra` running; `make migrate`, `make gen-api` and `make test` pass **before** changing anything.

**While working**
- Keep changes scoped to the task; if a change touches `core/`, shared config or another module, say so at the top
  of the PR. Follow `docs/PATTERNS.md`; copy the closest existing example.
- Small Conventional Commits (`feat(documents): ...`, `fix(...)`, `docs(...)`, `chore(...)`, `test(...)`); stage
...
- Migrations: pull main right before `make migration`; one per PR, message prefixed with the module; never edit a
  migration already on main; multiple heads → `flask --app app db merge heads -m "merge heads"` (via `conda run`).
- Route/schema change → `make gen-api` and fix type errors in your features.
- New dependency: pin it (`pkg==x.y.z`) in `backend/requirements*.txt` + `make env-update`, or
  `conda run -n ca-helper --cwd frontend npm install <pkg>`; mention it in the PR. Every new service/route gets
  tests; every new table gets seed data.
...
- Show a summary and **ask before `git push`**; draft the PR from `.github/pull_request_template.md` (`gh pr create`).
